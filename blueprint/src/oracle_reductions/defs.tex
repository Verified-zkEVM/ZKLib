...
\begin{definition}[Protocol Specification Components]
    ...
    Given a protocol specification $\mathsf{pSpec} : \ProtocolSpec\; n$, we define:
    \begin{itemize}
        \item $\mathsf{pSpec}.\mathsf{Dir}\; i := (\mathsf{pSpec}\; i).\mathsf{fst}$ extracts the direction of the $i$-th message.
        \item $\mathsf{pSpec}.\Type\; i := (\mathsf{pSpec}\; i).\mathsf{snd}$ extracts the type of the $i$-th message.
        \item $\mathsf{pSpec}.\MessageIdx := \{i : \Fin\; n \quotient \mathsf{pSpec}.\mathsf{Dir}\; i = \PtoVdir\}$ is the subtype of indices corresponding to prover messages.
        \item $\mathsf{pSpec}.\ChallengeIdx := \{i : \Fin\; n \quotient \mathsf{pSpec}.\mathsf{Dir}\; i = \VtoPdir\}$ is the subtype of indices corresponding to verifier challenges.
        \item $\mathsf{pSpec}.\mathsf{Message}\; i := (i : \mathsf{pSpec}.\MessageIdx) \to \mathsf{pSpec}.\Type\; i.\mathsf{val}$ is the type of all messages in the protocol.
        \item $\mathsf{pSpec}.\mathsf{Challenge}\; i := (i : \mathsf{pSpec}.\ChallengeIdx) \to \mathsf{pSpec}.\Type\; i.\mathsf{val}$ is the type of all challenges in the protocol.
    \end{itemize}
    \lean{ProtocolSpec.Dir, ProtocolSpec.getType, ProtocolSpec.MessageIdx, ProtocolSpec.ChallengeIdx, ProtocolSpec.Message, ProtocolSpec.Challenge}
\end{definition}

\begin{definition}[Transcript \& Related Definitions]
    \label{def:transcript_and_related_defs}
        Given protocol specification $\mathsf{pSpec} : \ProtocolSpec\; n$, we define:
    \begin{itemize}
        \item A \emph{transcript} up to round $k : \Fin\; (n + 1)$ is an element of type
                \[ \Transcript\; k\; \mathsf{pSpec} := (i : \Fin\; k) \to \mathsf{pSpec}.\Type\; (i^{\uparrow \Fin\; n}) \]
        where $i^{\uparrow \Fin\; n}$ denotes casting $i : \Fin\; k$ to $\Fin\; n$ (valid since $k \leq n + 1$).

        \item A \emph{full transcript} is $\FullTranscript\; \mathsf{pSpec} := (i : \Fin\; n) \to \mathsf{pSpec}.\Type\; i$.

        \item The type of all \emph{messages} from prover to verifier is
        \[ \mathsf{pSpec}.\mathsf{Messages} := \prod_{i : \mathsf{pSpec}.\MessageIdx} \mathsf{pSpec}.\mathsf{Message}\; i \]

        \item The type of all \emph{challenges} from verifier to prover is
        \[ \mathsf{pSpec}.\mathsf{Challenges} := \prod_{i : \mathsf{pSpec}.\ChallengeIdx} \mathsf{pSpec}.\mathsf{Challenge}\; i \]
    \end{itemize}
