...
noncomputable def foldingRange
   (degree : ℕ) [Nonempty ι] (domain : ι ↪ F) (f : ι → F) ( C : code F ι domain degree) : ℝ :=
    min (δᵣ(f,(toCode (toLinearCode C)))) (1 - Bstar (rate degree ι))

lemma folding
  [DecidableEq F] [Nonempty ι] (domain : ι ↪ F) (f : ι → F)
  (k : ℕ) (x : indexPow ι k) {degree : ℕ}
  (C1 : code F ι domain degree)
  (C2 : code F (indexPow ι k) (pow domain k) (degree / k))
  (δ : ℚ) (hδPos: δ > 0) (hδLt : δ < foldingRange degree domain f C1) :
    (PMF.uniformOfFintype F).toOuterMeasure { r : F |
            δᵣ((fold domain f k r),C2) ≤ δ} > err' F (degree/k) (rate degree ι) δ k
    := by sorry
