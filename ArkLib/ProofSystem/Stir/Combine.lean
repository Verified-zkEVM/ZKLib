...
lemma geometric_sum_units
{F : Type*} [Field F] [DecidableEq F] {r :  Units F} {a : ℕ} :
  ∑ j : Fin (a + 1), (r ^ (j:ℕ) : F) =
    if r = 1 then (a + 1 : F)
...
def ri {F : Type*} [Semiring F] {m : ℕ} (dstar : ℕ) (degs : Fin m → ℕ) (r : F) (i : Fin m) : F :=
r^(i.val + ∑ j < i, (dstar - degs j))

/-- Combine(d*, r, (f_0, d_0), …, (f_{m-1}, d_{m-1}))(x)
:= sum_{i=0}^{m-1} r_i * f_i(x) * ( sum_{l=0}^{d* - d_i -1} (r·x)^l ) -/
def combineInterm
  {F : Type*} [Semiring F] {m : ℕ} {ι : Type*} [Fintype ι] {domain : ι ↪ F} (dstar : ℕ) (r : F)
...
def conditionalExp
  {F : Type*} [Field F] [DecidableEq F] {ι : Type*} [Fintype ι] (domain : ι ↪ F)
  (dstar : ℕ) (degree : ℕ) (r : F) : ι → F :=
  fun x =>
    let q := r * (domain x)
...
def combineFinal
  {F : Type*} [Field F] [DecidableEq F] {m : ℕ} {ι : Type*} [Fintype ι] (domain : ι ↪ F)
  (dstar : ℕ) (r : F) (fs : Fin m → ι → F)
...
{F : Type*} [Semiring F] {ι : Type*} [Fintype ι] {domain : ι ↪ F}
...
/-- δ ∈ (0, min {1 − B⋆(ρ), 1 − ρ − 1/|L|}) -/
 noncomputable def combineRange
  ...
  {ι : Type*} [Fintype ι] {degree : ℕ} {domain : ι ↪ F}
...
lemma combine
  ...
  {ι : Type*} [Fintype ι] {domain : ι ↪ F} {L : Finset ι}
